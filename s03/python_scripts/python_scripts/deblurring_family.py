...
for epoch in range(num_epochs):
    ...
    for imgs, _ in trainloader:
        ...
        # (2)
        # conv_imgs = convolution_fft_torch(imgs, kernel_torch)
...
# (1): 230s runtime, ~18.7 - 5.7 SNR
# (2): 60s runtime, same SNR
# (3): 788s runtime
...
if __name__ == '__main__':
    ...
    print('---> Number of trainable parameters of supercnn: {}'.format(
    ...
    for i, data in enumerate(testloader, 0):
        ...
        for k in range(img_clean.shape[0]):
            ...
            SNR_noise[cpt] = SNR(img_clean[k:k + 1, 0].detach().cpu().numpy(),
                                 img_noisy[k:k + 1, 0].detach().cpu().numpy())
            cpt += 1
